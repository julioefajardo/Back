...
if ! ask "Este instalador só funcionará correctamente se for membro de ${bf}UNICAMP - Universidade Estadual de Campinas${n} e pertencer ao grupo: ${bf}eduroam-GS.${n}" "Continue" 1 ; then exit; fi
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:radius1.unicamp.br','DNS:radius2.unicamp.br'})
        server_name = 'unicamp.br'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
...
network={
  ...
  identity="${USER_NAME}"
  domain_suffix_match="unicamp.br"
  phase2="auth=PAP"
  password="${PASSWORD}"
}
EOFW
